...
public class DroneMovementScript : MonoBehaviour
{
    ...
    private float PropSpeed = 700f;
    private float PropSpeedIncrease = 2;
    ...
    void MovementUpDown()       //function that controles the height translation of the drone
    {
        ...
        {
            upForce = 150;      //force upward in Newton applied to the drone to accelerate it upward
            ...
            RotorG.transform.Rotate(0f, -1 * PropSpeedIncrease * PropSpeed * Time.fixedDeltaTime, 0f, Space.Self);
        ...
        {
            upForce = 50;     //force downward (because it's negative) in Newton applied to the drone which makes it accelerate downward
            RotorT.transform.Rotate(0f, -1 * PropSpeedIncrease * PropSpeed * Time.fixedDeltaTime, 0f, Space.Self);
            RotorY.transform.Rotate(0f, PropSpeedIncrease * PropSpeed * Time.fixedDeltaTime, 0f, Space.Self);
            RotorG.transform.Rotate(0f, PropSpeedIncrease * PropSpeed * Time.fixedDeltaTime, 0f, Space.Self);
            RotorH.transform.Rotate(0f, -1 * PropSpeedIncrease * PropSpeed * Time.fixedDeltaTime, 0f, Space.Self);
        }
    ...
    void MovementForward()      //function that controles the forward/backward translation of the drone
    {
        if (Input.GetAxis("Vertical") != 0)      //standard input from unity, relates to the keys W & S for translating vertically/forward
        {
            ...
            tiltAmountForward = Mathf.SmoothDamp(tiltAmountForward, 20 * Input.GetAxis("Vertical"), ref tiltVelocityForward, 0.00000001f);     //makes the drone tilt in forward or backward when moving forward or backward and slowly return to 0 when stationairy. change the number of the second argument to change the tilt angle (must stay positive)
    ...
    void ClampingSpeedValues()      //function that limits the speed of the different translations and stops the drone after initializing a translation
    {
        if (Mathf.Abs(Input.GetAxis("Vertical")) > 0.2f && Mathf.Abs(Input.GetAxis("Horizontal")) > 0.2f)
        {
            ourDrone.velocity = Vector3.ClampMagnitude(ourDrone.velocity, Mathf.Lerp(ourDrone.velocity.magnitude, 0f, Time.deltaTime * 1f));  //change the number in the last argument to change the decaleration rate (must be positive)
        }
        ...
        if (Mathf.Abs(Input.GetAxis("Vertical")) < 0.2f && Mathf.Abs(Input.GetAxis("Horizontal")) < 0.2f)
        {
            ourDrone.velocity = Vector3.SmoothDamp(ourDrone.velocity, Vector3.zero, ref velocityToSmoothDampToZero, 1f);  //change the number in the last argument to change the decaleration rate (must be positive)
        }
    }

    //--------------------------------------------------------------------------------------------------------------------------------------------------------------------
    private float sideMovement = 15f;    //change this float to change the sideways speed of the drone, the higher the number, the faster the drone goes (must be positive)
    ...
    void DroneSound()
    {
        droneSound.pitch = 0.7f + (ourDrone.velocity.magnitude / 10);
    }
}
